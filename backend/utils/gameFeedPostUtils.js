/** Go Fish feed posts — detect, dedupe, and enrich when cardGameData was not persisted. */
...
export function parseCardGameDataRaw(raw) {
  if (!raw) return null
  try {
...
export function getGameRoomIdFromPost(post) {
  ...
  if (!raw) return ''
  const data = parseCardGameDataRaw(raw)
  return data?.roomId != null ? String(data.roomId).trim() : ''
}

/** Stable key for two Go Fish posts (one per player) when roomId is missing. */
export function getGoFishPairDedupeKey(post) {
  ...
  const pair = [authorName, opponentName].sort().join('|')
  return `gofish:${pair}`
}
...
export function getGameFeedDedupeKey(post) {
  const roomId = getGameRoomIdFromPost(post)
  if (roomId) return `room:${roomId}`
  return getGoFishPairDedupeKey(post)
}

/** Same game card always shows players in the same slots (sorted by user id). */
export function normalizeCardGamePlayers(data) {
  if (!data) return null
...
export function pickPreferredGoFishFeedPost(kept, candidate) {
  const keptHas = !!kept?.cardGameData
  const candHas = !!candidate?.cardGameData
...
export function mergeGoFishFeedPostData(kept, incoming) {
  const preferred = pickPreferredGoFishFeedPost(kept, incoming)
  const other = preferred === kept ? incoming : kept
  if (!other?.cardGameData || preferred?.cardGameData) return preferred
  return { ...preferred, cardGameData: other.cardGameData }
}

export function buildMinimalCardGameDataFromText(post) {
  if (!isGoFishFeedPost(post)) return null
  const parsed = parseCardGameDataRaw(post?.cardGameData)
  if (parsed) return normalizeCardGamePlayers(parsed)
  const m = String(post?.text || '').match(/Playing Go Fish with\s+(.+?)\s*🃏/i)
  const opponentName = m?.[1]?.trim()
  const pb = post?.postedBy
  return normalizeCardGamePlayers({
    player1: {
